...
class ThumbhashEncoder:
    def __init__(self, img: Image):
        ...
        self.width, self.height, self.dims = self.data.shape
        assert self.dims == 4  # RGBA
        self.hash = ThumbhashEncoder.rgba_to_thumb_hash(self.width, self.height, self.data)
    ...
    def rgba_to_thumb_hash(w: int, h: int, data: np.ndarray):
        ...
        l_dc, l_ac, l_scale = ThumbhashEncoder.encode_channel(l, max(3, lx), max(3, ly), w, h)
        p_dc, p_ac, p_scale = ThumbhashEncoder.encode_channel(p, 3, 3, w, h)
        q_dc, q_ac, q_scale = ThumbhashEncoder.encode_channel(q, 3, 3, w, h)
        isLandscape = w > h
        ...
        ac_index = 0
        if has_alpha:
            a_dc, a_ac, a_scale = ThumbhashEncoder.encode_channel(alpha, 5, 5, w, h)
    ...
    def to_numpy(self) -> np.ndarray:
        return np.array(self.hash, dtype=np.uint8)
    ...
    def to_base64(self) -> bytes:
        return b64encode(self.to_numpy())

    def __str__(self) -> str:
        return f'<ThumbHash "{self.to_hexstring()} >'
